...
export const fetchTodos = (userId) => (dispatch) => {
  ...
  SecureStore.getItemAsync('userInfo')
    .then((userdata) => {
      ...
      return API.get(`/users/${userId}/todos`, { headers: { authorization: token } })
        .then(response => {
...
export const createTodo = (userId, text) => (dispatch) => {
  ...
  SecureStore.getItemAsync('userInfo')
    .then((userdata) => {
      const userInfo = JSON.parse(userdata);
      const { token } = userInfo;
      return API.post(`/users/${userId}/todos`, { text }, { headers: { authorization: token } })
        .then(response => {
...
export const updateTodo = (userId, todoUpdate) => (dispatch) => {
  ...
  SecureStore.getItemAsync('userInfo')
    .then((userdata) => {
      const userInfo = JSON.parse(userdata);
      const { token } = userInfo;
      return API.put(`/users/${userId}/todos/${todoUpdate.id}`, { text: todoUpdate.text }, { headers: { authorization: token } })
        .then(response => {
